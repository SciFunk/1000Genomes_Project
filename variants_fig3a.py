...
pop_totals = {'ESN': 99,'GWD': 113,'LWK': 99,'MSL': 85,'YRI': 108,'ACB': 96,
              'ASW': 61,'CLM': 94,'MXL': 64,'PEL': 85,'PUR': 104,'CDX': 93,
              'CHB': 103,'CHS': 105,'JPT': 104,'KHV': 99,'CEU': 99,'GBR': 91,
              'FIN': 99,'IBS': 107,'TSI': 107,'BEB': 86,'GIH': 103,'ITU': 102,
              'PJL': 96,'STU': 102}
...
with gzip.open(sys.argv[1]) as data: #instead of data = gzip.open((sys.argv[1]),"rb"), this will read file line-by-line
  for line in data:
      if line.startswith('##'): #skip all ~250 comment lines
        ...
        percent_dict = {k: float(blank_dict[k])/pop_totals[k] for k in pop_totals.viewkeys() & blank_dict}
        for key in percent_dict:
            if percent_dict[key] > 0.049:
                final_dict[key] += 1
